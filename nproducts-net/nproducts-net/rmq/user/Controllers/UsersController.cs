...
using UserService1.Contracts.v1.Requests;
...
namespace UserService1.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("bhakta")]
        public async Task<IActionResult> Bhakta()
        {
            var newRequest = new BhaktaPubishRequest
            {
                BhaktaMessage = "I am Bhakta published to as many endpoints that are listening",
            };

            // use the message type (PubishRequest) to distribute itself to however many consumers are out there
            await _publishEndpoint.Publish<BhaktaPubishRequest>(newRequest);
